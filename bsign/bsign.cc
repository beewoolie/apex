/* bsign.cxx
     $Id: bsign.cc,v 1.32 2003/08/21 07:13:07 elf Exp $
...
#define CB_HASH		(20)		   // Length of hash data

size_t rewrite_elf32 (char* pb, size_t cb, int fh, FIXUP32& fixup);
bool is_zeroed (const char* pb, size_t cb);
		 ...
		 bool fCreateCert)
{
  ...
  memcpy (pbHash, rgb, CB_HASH);
  free (hd);

  if (!fCreateCert)
    return rgbSignature;

  char* rgbCert = create_digital_signature (rgbSignature, cbHeader + CB_HASH);
  if (rgbCert) {
    size_t cbCert = ((unsigned char*)rgbCert)[0]*256
      + ((unsigned char*)rgbCert)[1] + 2;
    memcpy (pbHash + CB_HASH, rgbCert, cbCert);
    free (rgbCert);
  }
  else {
    delete rgbSignature;
...
/* is_elf_rewritten
   ...
   returns true if this file is an elf file and has a signature
   section.  The pInfo structure is initialized with appropriate
   pointers if the signature section is found.  Note that this
   function does not distinguish between non-elf files and elf files
   that have no signature section.

*/

bool is_elf_rewritten (char* pb, size_t cb, SigInfo* pInfo)
{
  if (!pInfo || !is_elf (pb, cb))
    return false;
  ...
  for (int i = 0; i < _v (hdr.cEntrySection); ++i) {
    if (_v (rgSection[i].sectiontype) == unsigned32 (ELF_SECTION_SIGNATURE)) {
      pInfo->ibSection = _v (rgSection[i].ib);
      pInfo->cbSection = _v (rgSection[i].cb);
      return true;
    }
  }
  return false;
		       ...
		       SigInfo* pInfo)
{
  ...
  if (!pInfo)
    pInfo = (SigInfo*) alloca (sizeof (SigInfo));

  if (!is_elf_signed (pb, cb, pInfo))
    return fExpectSignature ? nosignature : nohash;
  if (pb[pInfo->ibSection] == '#')
    pInfo->version = atoi (&pb[pInfo->ibSection + 1]);
  for (pInfo->ibHash = pInfo->ibSection; 
       pInfo->ibHash < pInfo->ibSection + pInfo->cbSection - CB_HASH;
       ++pInfo->ibHash)
    if (pb[pInfo->ibHash] == '\n') {
      ++pInfo->ibHash;
      break;
    }
  pInfo->cbHash = CB_HASH;
  if (fExpectSignature
      && pInfo->ibHash == pInfo->ibSection + pInfo->cbSection - CB_HASH)
    return nosignature;		

				// Compute hash
  char* rgbSignature = sign_file (pb, cb, pInfo->ibSection, 
				  pInfo->cbSection, false);
  char* pbSignature;
  for (pbSignature = rgbSignature;
    ...
    if (*pbSignature == '\n') {
      ...
      break;
				...
				// Check for zero'd unused bytes
  pInfo->cbSignature = (((unsigned char*)pb)[pInfo->ibHash + CB_HASH] << 8) 
    | ((unsigned char*)pb)[pInfo->ibHash + CB_HASH + 1];
  bool fBadCertLength = (pInfo->cbSignature > pInfo->cbSection
			 - (pInfo->ibHash - pInfo->ibSection + CB_HASH + 2));
  if (!fExpectSignature && fBadCertLength)
    return unusednotzero;
  if (!fBadCertLength
      && !is_zeroed (pb + pInfo->ibHash + CB_HASH
		     + (pInfo->cbSignature ? pInfo->cbSignature + 2 : 0), 
		     pInfo->cbSection
		     - (pInfo->ibHash - pInfo->ibSection + CB_HASH
			+ (pInfo->cbSignature ? pInfo->cbSignature + 2 : 0))))
    return unusednotzero;
  if (!fBadCertLength && pInfo->cbSignature)
    pInfo->ibSignature = pInfo->ibHash + CB_HASH + 2;

				// Check hash
  eExitStatus result = 
    ((memcmp (pbSignature, pb + pInfo->ibHash, CB_HASH) != 0)
     ? badhash : noerror);
  free (rgbSignature);
  ...
  if (pInfo->cbSignature == 0 || fBadCertLength)
    return badsignature;
  return verify_digital_signature (pb + pInfo->ibSection,
				   pInfo->ibHash - pInfo->ibSection
				   + CB_HASH, 
				   pb + pInfo->ibHash + CB_HASH + 2, 
				   pInfo->cbSignature);
}
...
/* hash_elf
   ...
   *** Removed fhNew parameter.  We only rewrite the signature
   *** section.  Instead, we must receive a valid SigInfo that
   *** delineates the Section.
...
eExitStatus hash_elf (char* pb, size_t cb, int fh, SigInfo* pInfo, bool fSign)
{
  check_byte_sex (pb);		// May be redundant

  char* rgbSignature
    = sign_file (pb, cb, pInfo->ibSection, pInfo->cbSection, fSign);

  //  printf ("writing signature block to %x %x\n", 
  //	  pInfo->ibSection, pInfo->cbSection);

  if (rgbSignature) {
    lseek (fh, pInfo->ibSection, SEEK_SET);
    write (fh, rgbSignature, pInfo->cbSection);
    delete rgbSignature;
    return noerror;
  }
...
/* is_zeroed
   ...
   returns true if all bytes in the range are zero.
...
/* rewrite_elf
   ...
   reads the mmap'd file (pb,cb) and writes a new file to fhNew that
   has an empty signature section.  It elides the existing signature
   section if found one is found.  The SigInfo structure is return
   with updated data for the new file.  Adding digital signatures is
   performed elsewhere.

   We assume that the is_elf_header() function has already been called
   and, therefore, the byte sex is established.
...
eExitStatus rewrite_elf (char* pb, size_t cb, int fhNew, SigInfo* pInfo)
{
  ...
  pInfo->ibSection = fixupSignature.ibNew;
  pInfo->cbSection = fixupSignature.cbNew;
  return noerror;
}
...
/* rewrite_elf32
   ...
   performs the most essential part of bsign, rewriting the executable
   with a new section.  The mmap'd file (pb,cb) is rewritten to the
   filehandle fh with a new section the size given by fixupSignature.
   The offset of this new section is returned in fixupSignature.  An
   existing signature section that is at least as large as the one
   requested will be retained.  Note that it is possible for the size
   in fixupSignature to be larger than that requested.  The signature
   section is always zeroed. 
   ...
   The return value is -1 when there's an error during rewrite.
   Otherwise, it is the size of the new file.
...
#if defined (TALK)
      printf ("%8lu l%8d->%8lu from file\n", 
	      (unsigned long) ibFile, cb, (unsigned long)(ib + cb));
#endif
