/* elf.cc
     $Id: elf.cc,v 1.3 2003/08/11 07:45:04 elf Exp $
   ...
   Interface to the ELF file format.  It copes with endian differences
   as well as pointer width differences.
...
typedef enum {
  byte_4 = 1,			// Four bytes
  byte_10, 			// Ten bytes
  unsigned_8,			// Unsigned 8 bit
  unsigned_16,			// Unsigned 16 bit
  unsigned_32,			// Unsigned 32 bit
  address,			// Address pointer for bitcass
  index,			// Counter (word sized) for bitclass
} ELF_FIELD_SIZE;
...
typedef struct {
  unsigned32 field;
  int width;
  int ib;			// Computed offset and width
  int cb;			//   for this field
} FIELD;

FIELD rgField[] = {
// Header
  { elf_header  | rgbID,		byte_4 },
  { elf_header  | bitclass,		unsigned8 },
  { elf_header	| byteorder,		unsigned8 },
  { elf_header  | rgbMagic,		byte_10 },
  { elf_header	| filetype,		unsigned16 },
  { elf_header	| cpu,			unsigned16 },
  { elf_header	| version,		unsigned32 },
  { elf_header  | addrEntry,		address },
  { elf_header  | ibHdrProgram,		index },
  { elf_header  | ibHdrSection,		index },
  { elf_header  | flags,		unsigned32 },
  { elf_header  | cbHeader,		unsigned16 },
  { elf_header  | cbEntryProgram,	unsigned16 },
  { elf_header  | cEntryProgram,	unsigned16 },
  { elf_header  | cbEntrySection,	unsigned16 },
  { elf_header  | cEntrySection,	unsigned16 },
  { elf_header  | iSectionNames,	unsigned16 },
  { elf_program | programtype,		unsigned32 },
  { elf_program | ib,			index },
  { elf_program | addrVirtual,		address },
  { elf_program | addrPhysical,		address },
  { elf_program | cbFile,		index },
  { elf_program | cbMemory,		index },
  { elf_program | flags,		unsigned32 },
  { elf_program | alignment,		unsigned32 },
  { elf_section | ibName,		unsigned32 },
  { elf_section | sectiontype,		unsigned32 },
  { elf_section | flags,		unsigned32 },
  { elf_section | addr,			address },
  { elf_section | ib,			index },
  { elf_section | cb,			index },
  { elf_section | iLink,		unsigned32 },
  { elf_section | info,			unsigned32 },
  { elf_section | alignment,		unsigned32 },
  { elf_section | cbEntry,		unsigned32 },
  { elf_symbol  | ibName,		unsigned32 },
  { elf_symbol  | value,		unsigned32 },
  { elf_symbol  | cb,			index },
  { elf_symbol  | info,			unsigned8 },
  { elf_symbol  | other,		unsigned8 },
  { elf_symbol  | iSection,		unsigned16 },
  { 0 }
};
		 ...
		 bool fCreateCert)
{
  ...
  if (fCreateCert) {
    ...
    if (rgbCert) {
      size_t cbCert = rgbCert[0]*256 + rgbCert[1] + 2;
...
eExitStatus check_elf (char* pb, size_t cb, bool fExpectSignature)
{
  ...
  size_t cbCert = (pb[ibHash + 20] << 8) | pb[ibHash + 21];
