/* ds.cxx
     $Id: ds.cc,v 1.20 2003/08/11 07:38:01 elf Exp $
...
char* create_digital_signature (const char* pb, size_t cb)
{
  ...
  sprintf (sz, PG_PROGRAM " --no-greeting "
	   ...
	   exsPassPhrase.fd_write (), 
	   g_arguments.szPGOptions ? g_arguments.szPGOptions : "");
				      ...
				      const char* pbCert, size_t cbCert)
{
  ...
  if (g_arguments.fDebug)
    fprintf (stderr, "%s: creating temporary '%s' for data on verify\n", 
	     g_arguments.szApplication, g_szPathTempVerify);
  ...
  if (result && g_arguments.fVerbose)
